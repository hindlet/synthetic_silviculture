...
/// spawns terrain from a given heightmap
pub fn create_heightmap_terrain(
...
) -> (Terrain, (f32, RangeInclusive<f32>, RangeInclusive<f32>), (Vec<Vector3>, Vec<u32>)) {
    ...
    (Terrain{collider: collider}, (max_height, (centre.x - size)..=(centre.x + size), (centre.z - size)..=(centre.z + size)), (vertices, indices))
}
...
pub fn create_flat_terrain(
    ...
    (Terrain{collider: collider}, (centre.y, (centre.x - half_size)..=(centre.x + half_size), (centre.z - half_size)..=(centre.z + half_size)), (vertices, indices))
}