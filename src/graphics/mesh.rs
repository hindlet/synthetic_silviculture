...
impl Mesh {
    ...
    pub fn flat_shaded(&self) -> Mesh {
        ...
        for i in (0..self.indices.len()).step_by(3) {
            ...
            new_verts.push(Vertex::from(v_one));
            new_verts.push(Vertex::from(v_two));
            new_verts.push(Vertex::from(v_thr));
    ...
    pub fn flat_shade_components(in_verts: Vec<Vertex>, in_inds: Vec<u32>) -> (Vec<Vertex>, Vec<Normal>, Vec<u32>){
        let mut new_verts: Vec<Vertex> = Vec::new();
        let mut new_normals: Vec<Normal> = Vec::new();
...
impl Into<(Vec<Vertex>, Vec<Normal>, Vec<u32>)> for Mesh{
    fn into(self) -> (Vec<Vertex>, Vec<Normal>, Vec<u32>) {
